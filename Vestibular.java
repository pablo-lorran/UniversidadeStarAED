...
public class Vestibular {
  ...
  public void arquivoEntrada(String arqEntrada) {
    try {
      Scanner arqLeitura = new Scanner(
        new FileInputStream(arqEntrada),
        "UTF-8"
      );
      ...
      System.out.println(
        "Erro na conversão de um valor numérico no arquivo de entrada!"
      );
    }
  }
      ...
      for (int i = 1; i <= qtdCursos; i++) {
        ...
        if (curso != null) {
          ...
          if (!listaAprovados.isEmpty()) {
            Candidato ultimoAprovado = listaAprovados.get(
              listaAprovados.size() - 1
            );
            arqEscrita.format(
              "%s %.2f\n",
              curso.getNomeCurso(),
              ultimoAprovado.getNotaMedia()
            );
            arqEscrita.format("Selecionados\n");
            Set<String> candidatosEscritos = new HashSet<>(); // Conjunto para armazenar os candidatos já escritos
            for (Candidato candidato : listaAprovados) {
              String infoCandidato = String.format(
                "%s %.2f\n",
                candidato.getNomeCandidato(),
                candidato.getNotaMedia()
              );
              if (!candidatosEscritos.contains(infoCandidato)) {
                arqEscrita.format(infoCandidato);
                candidatosEscritos.add(infoCandidato);
              }
            }
  ...
  public void calcularResultado() {
    ...
    for (int i = 0; i < qtdCandidatos; i++) {
      ...
      if (
        op1Curso != null && candidato.getNotaMedia() >= op1Curso.getNotaCorte()
      ) {
